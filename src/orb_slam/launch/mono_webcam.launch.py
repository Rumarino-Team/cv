...
def generate_launch_description():
    ...
    odom_topic_arg = DeclareLaunchArgument(
        "odom_topic",
        default_value="orb_slam3/camera_odom",
        description="Output camera odometry topic",
    )
    ...
    world_frame_arg = DeclareLaunchArgument(
        "world_frame_id", default_value="world", description="TF world frame ID"
    )

    camera_frame_arg = DeclareLaunchArgument(
        "camera_frame_id", default_value="camera", description="TF camera frame ID"
    )

    queue_size_arg = DeclareLaunchArgument(
        "queue_size", default_value="10", description="ROS subscription queue size"
    )
    ...
    image_width_arg = DeclareLaunchArgument(
        "image_width", default_value="640", description="Camera image width"
    )

    image_height_arg = DeclareLaunchArgument(
        "image_height", default_value="480", description="Camera image height"
    )

    framerate_arg = DeclareLaunchArgument(
        "framerate", default_value="30.0", description="Camera framerate"
    )
    ...
    return LaunchDescription(
        [
            ...
            TimerAction(period=5.0, actions=[orb_slam3_node]),
        ]
    )
