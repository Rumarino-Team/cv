...
class CameraCalibrator:
    def __init__(self, pattern_size=(9, 6), square_size=0.025):
        ...
        self.objp[:, :2] = np.mgrid[0 : pattern_size[0], 0 : pattern_size[1]].T.reshape(
            -1, 2
        )
    ...
    def find_chessboard(self, image, show=True):
        ...
        if ret:
            ...
            cv2.drawChessboardCorners(
                display_img, self.pattern_size, corners_refined, ret
            )

            if show:
                cv2.putText(
                    display_img,
                    f"Pattern found! Total: {len(self.objpoints)}",
                    (10, 30),
    ...
    def calibrate_from_video(self, video_source=0, num_frames=20, skip_frames=10):
        ...
        print(
            f"- Pattern size: {self.pattern_size[0]}x{self.pattern_size[1]} (cols x rows)"
        )
        ...
        while capture_count < num_frames:
            ...
            cv2.putText(
                display_img,
                f"Captures: {len(self.objpoints)}/{num_frames}",
                (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.7,
                (255, 255, 0),
            ...
            if key == ord("q"):
                ...
                if len(self.objpoints) >= 3:
                    ...
                    print(
                        f"Need at least 3 images, currently have {len(self.objpoints)}"
                    )
        ...
        if len(self.objpoints) < 3:
            print(
                f"Not enough calibration images. Got {len(self.objpoints)}, need at least 3"
            )
            return False
    ...
    def calibrate_from_images(self, image_pattern):
        ...
        for idx, fname in enumerate(images):
            ...
            if ret:
                ...
                print(
                    f"✗ [{idx+1}/{len(images)}] {os.path.basename(fname)} - Pattern not found"
                )
    ...
    def calibrate(self):
        ...
        print(
            f"k1={self.dist_coeffs[0][0]:.6f}, k2={self.dist_coeffs[0][1]:.6f}, "
...
""",
    ...
    parser.add_argument(
        "--fps", type=int, default=30, help="Camera FPS for YAML output (default: 30)"
    )
    parser.add_argument(
        "--rgb",
        type=int,
    ...
    calibrator = CameraCalibrator(
        pattern_size=pattern_size, square_size=args.square_size
    )
    ...
    if args.input == "0":
        ...
        success = calibrator.calibrate_from_video(
            args.input, num_frames=args.num_frames
        )
    else:
    ...
    if success:
        # Save results
        calibrator.save_to_orbslam_yaml(
            args.output, camera_fps=args.fps, camera_rgb=args.rgb
        )
        print(
